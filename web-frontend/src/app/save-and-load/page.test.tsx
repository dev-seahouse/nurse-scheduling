...
describe('SaveAndLoadPage', () => {
  ...
  it('keeps edited YAML open when semantic import validation rejects replacement', async () => {
    const user = userEvent.setup();
    const diagnostic = {
      ...contractedHoursDiagnostic('yaml-import'),
      repair: 'Correct the YAML at the reported path, or remove hoursContract to import it as a generic Shift Count.',
    };
    loadFromYaml.mockReturnValueOnce({
      ok: false,
      message: `${diagnostic.message} (${diagnostic.path}) ${diagnostic.repair}`,
      diagnostic,
    });
    ...
    expect(screen.getAllByText(/remove hoursContract to import it as a generic Shift Count/i))
      .toHaveLength(2);
  ...
  it('blocks an invalid plain download without changing state and succeeds after a retry', async () => {
    const user = userEvent.setup();
    let invalid = true;
    mockGenerateYamlFromState.mockImplementation((_state, options) => {
      if (options?.contractedHoursBoundary === 'yaml-download' && invalid) {
        throw new ContractedHoursBoundaryError(
          contractedHoursDiagnostic('yaml-download'),
        );
      }
    ...
    expect(screen.getByRole('link', { name: 'Review Contracted Hours' }))
      .toHaveAttribute('href', '/shift-counts');
    expect(filterAutoGeneratedState.mock.lastCall?.[0]).toMatchObject({
      description: 'baseline',
    });
    ...
    expect(screen.queryByText('Contracted Hours needs attention')).not.toBeInTheDocument();
  });

  it('uses named validation gates for clipboard and anonymized YAML actions', async () => {
    const user = userEvent.setup();
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.defineProperty(navigator, 'clipboard', {
      value: { writeText },
