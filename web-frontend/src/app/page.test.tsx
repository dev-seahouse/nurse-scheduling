...
describe('Home', () => {
  beforeEach(() => {
    ...
    loadFromYaml.mockReset().mockReturnValue({ ok: true });
    push.mockReset();
    vi.unstubAllGlobals();
  });
...
`),
  ...
  it('keeps the dialog open and reports an example the import rejects', async () => {
    const user = userEvent.setup();
    loadFromYaml.mockReturnValue({ ok: false, message: 'The example could not be imported.' });
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, text: () => Promise.resolve('apiVersion: alpha\n') }));
    render(<Home />);
    ...
    expect(await screen.findByText('The example could not be imported.')).toBeInTheDocument();
    expect(screen.getByText(/Load the 87-person example/)).toBeInTheDocument();
  });
});
