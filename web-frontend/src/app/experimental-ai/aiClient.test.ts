...
describe('AI client', () => {
  ...
  it('creates a browser-owned session', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.nursescheduling.org/ai/sessions', {
      ...
      body: JSON.stringify({ schedule_yaml: 'description: test', singapore_holidays: [] }),
    });
  });

  it('sends the cached Singapore holidays when creating and updating a session', async () => {
    const holidays = [{ date: '2025-12-25', name: 'Christmas Day', isObserved: false }];
    ...
    await createSession('description: test', null, '/ai', holidays);
    await updateSessionSchedule('session-id', 'description: newer', null, '/ai', holidays);

    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toEqual({
      schedule_yaml: 'description: test',
      singapore_holidays: holidays,
    });
    expect(JSON.parse(fetchMock.mock.calls[1][1].body)).toEqual({
      schedule_yaml: 'description: newer',
      singapore_holidays: holidays,
    });
  });
