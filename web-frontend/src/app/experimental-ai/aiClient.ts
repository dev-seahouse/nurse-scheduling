...
export async function createSession(
  ...
  singaporeHolidays: SingaporeHolidayEntry[] = [],
): Promise<string> {
  const response = await fetch(`${endpoint}/sessions`, {
    method: 'POST',
    credentials: 'include',
    headers: authorizedHeaders(authToken, { 'Content-Type': 'application/json' }),
    // The AI rebuilds holiday date groups from the entries this browser has cached.
    body: JSON.stringify({ schedule_yaml: scheduleYaml, singapore_holidays: singaporeHolidays }),
  });
  if (!response.ok) throw await responseError(response);
...
export async function updateSessionSchedule(
  ...
  const response = await fetch(`${endpoint}/sessions/${encodeURIComponent(sessionId)}/schedule`, {
    ...
    credentials: 'include',
    headers: authorizedHeaders(authToken, { 'Content-Type': 'application/json' }),
    // Omitted holidays keep the entries the session already holds.
    body: JSON.stringify({ schedule_yaml: scheduleYaml, singapore_holidays: singaporeHolidays }),
  });
  if (!response.ok) throw await responseError(response);
}
