...
const mockSingaporeHolidays = vi.hoisted(() => [{ date: '2025-12-25', name: 'Christmas Day', isObserved: false }]);
...
vi.mock('@/hooks/useSingaporeHolidays', () => ({
  useSingaporeHolidays: () => ({ status: 'ready', entries: mockSingaporeHolidays, error: null, refetch: vi.fn() }),
}));
...
describe('ExperimentalAiPage', () => {
  beforeEach(() => {
    ...
    mockLoadFromYaml.mockReset().mockReturnValue({ ok: true });
  ...
  describe('proposals', () => {
    ...
    it('keeps a proposal whose schedule the app rejects on import', async () => {
      mockLoadFromYaml.mockReturnValue({ ok: false, message: 'The proposed contracted hours are invalid.' });
      const user = userEvent.setup();
      render(<ExperimentalAiPage />);
      await ask(user);
      ...
      expect(await screen.findByText('The proposed contracted hours are invalid.')).toBeInTheDocument();
