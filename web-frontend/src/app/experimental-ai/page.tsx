...
export default function ExperimentalAiPage() {
  ...
  const { entries: singaporeHolidayEntries } = useSingaporeHolidays();
  // The holiday list the session last received, compared by identity because the hook replaces it on load.
  const syncedHolidaysRef = useRef<SingaporeHolidayEntry[] | null>(null);
  ...
  const sendRequest = async (
    ...
    try {
      ...
      if (sessionId === null) {
        sessionId = await createSession(scheduleYaml, authToken, sessionEndpoint, singaporeHolidayEntries);
        ...
        || syncedHolidaysRef.current !== singaporeHolidayEntries
      ) {
        // The schedule can change elsewhere in the app between questions, and the
        // holidays can finish loading after the session was created.
        await updateSessionSchedule(sessionId, scheduleYaml, authToken, sessionEndpoint, singaporeHolidayEntries);
      }
      syncedScheduleRef.current = scheduleYaml;
      syncedHolidaysRef.current = singaporeHolidayEntries;
  ...
  const applyProposal = async () => {
    ...
    try {
      ...
      const loadResult = loadFromYaml(yaml.load(approvedYaml));
