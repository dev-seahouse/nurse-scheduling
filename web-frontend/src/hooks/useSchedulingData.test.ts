  ...
  const state = {
    ...
    dates: {
      ...
      groups: [
        ...
        { id: 'NON-WORKDAY', members: [], description: 'Non-Work Days' },
        { id: 'PH', members: [], description: 'Public Holidays' },
      ],
    },
    people: {
...
describe('useSchedulingData', () => {
  ...
  it('falls back to the default state and logs when localStorage.getItem throws', async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.dateData.groups.filter(group => !group.isAutoGenerated).map(group => group.id))
        .toEqual(['WORKDAY', 'NON-WORKDAY', 'PH']);
