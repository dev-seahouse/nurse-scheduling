...
import { canonicalizeWorkingTime, collectStructuralImportErrors } from '@/utils/schedulingStructuralBoundary';
...
export function loadStateFromStorage(): HistoryState {
  ...
  try {
    ...
    const history = parsedHistoryState.history.map(normalizeStateForFrontend);

    // No-migration fence (WT3): stale development storage that predates the
    // current schema — retired unit markers or invalid working-time shapes —
    // is rejected wholesale and reset to the safe default, never upgraded,
    // stripped, or loaded partially. Validate every stored state so a single
    // invalid entry cannot be revived as live current-schema data.
    const storedStates = [currentState, ...history];
    for (const state of storedStates) {
      if (collectStructuralImportErrors(state).length > 0) {
        return createDefaultHistoryState();
      }
    }

    for (const state of storedStates) {
      canonicalizeWorkingTime(state.shiftTypes.items);
    }
