...
export function useSchedulingDataInternal() {
  ...
  const addItem = (
    ...
    const newItem = dataType === DataType.PEOPLE
      ...
      : dataType === DataType.SHIFT_TYPES
        ? { id, description: description || '', ...(durationMinutes != null ? { durationMinutes } : {}) }
  ...
  const updateItem = (
    ...
    // First update the item's ID and description. For shift types, the
    // authoring-only durationMinutes is set from the draft (undefined clears it).
    const updatedItems = data.items.map(item =>
      item.id === oldId
        ? {
            ...
            ...(dataType === DataType.SHIFT_TYPES ? { durationMinutes } : {})
          }
        : item
    );
