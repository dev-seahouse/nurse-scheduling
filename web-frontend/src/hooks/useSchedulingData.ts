...
// passing `undefined` leaves whatever the item already has untouched (so ID- and
// description-only edits never wipe the times). An empty object clears them.
export type ShiftTypeWorkingTime = Pick<
  Item,
  'durationMinutes' | 'startTime' | 'endTime' | 'restMinutes'
>;

// Keep only the defined working-time fields, so a freshly-added shift type never
// carries `key: undefined` noise (updates set all four explicitly to clear).
function pickDefinedWorkingTime(workingTime: ShiftTypeWorkingTime): ShiftTypeWorkingTime {
  const picked: ShiftTypeWorkingTime = {};
  if (workingTime.durationMinutes != null) picked.durationMinutes = workingTime.durationMinutes;
  if (workingTime.startTime) picked.startTime = workingTime.startTime;
...
export function useSchedulingDataInternal() {
  ...
  const addItem = (
    ...
    const newItem = dataType === DataType.PEOPLE
      ...
      : dataType === DataType.SHIFT_TYPES
        ? { id, description: description || '', ...(workingTime ? pickDefinedWorkingTime(workingTime) : {}) }
  ...
  const updateItem = (
    ...
    // First update the item's ID and description. For shift types, an explicit
    // working-time payload replaces all four durable fields at once (an undefined
    // field clears it); passing no payload leaves the stored times untouched so
    // ID/description-only edits stay durable.
    const updatedItems = data.items.map(item =>
      item.id === oldId
        ? {
            ...
            ...(dataType === DataType.SHIFT_TYPES && workingTime !== undefined
              ? {
                  durationMinutes: workingTime.durationMinutes,
                  startTime: workingTime.startTime,
                  endTime: workingTime.endTime,
                  restMinutes: workingTime.restMinutes,
                }
              : {})
          }
        : item
