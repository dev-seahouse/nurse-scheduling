...
export interface Item {
  ...
  // Shift types only, all authoring-only (ignored by the solver) — mirror of the
  // durable backend fields (WT1, models.py:74-83). `durationMinutes` is the paid
  // working minutes that feeds coefficient auto-fill; `startTime`/`endTime` are
  // "HH:MM" clock times and `restMinutes` the unpaid break, from which WT4 derives
  // `durationMinutes`. All optional — absent ⇒ legacy behavior.
