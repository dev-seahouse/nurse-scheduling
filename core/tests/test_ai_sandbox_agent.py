...
from nurse_scheduling.ai.sandbox_agent import (
    REFERENCE_SCHEMAS,
    REFERENCE_SINGAPORE_HOLIDAY_ENTRIES,
...
from nurse_scheduling.ai.schema import load_singapore_holidays_reference, load_user_guide_references
...
def _collect(
    ...
    *,
    ...
    singapore_holidays: Sequence[dict] = (),
    **limit_overrides,
) -> list:
...
def test_one_turn_hydrates_runs_reads_validates_proposes_and_closes():
    ...
    assert b"buildSingaporeHolidayGroups" in backend.files[REFERENCE_SCHEMAS["singapore-holidays"]]
    assert json.loads(backend.files[REFERENCE_SINGAPORE_HOLIDAY_ENTRIES]) == []
...
def test_hydration_writes_browser_holiday_entries_sorted_by_date():
    factory = FakeSandboxFactory()
    provider = ScriptedProvider(
        [ToolCallRequest((ToolCall("call-1", READ_TOOL, json.dumps({"path": REFERENCE_SINGAPORE_HOLIDAY_ENTRIES})),))],
        [TextDelta("Read.")],
    )
    christmas = {"date": "2025-12-25", "name": "Christmas Day", "isObserved": False}
    deepavali = {"date": "2025-10-20", "name": "Deepavali", "isObserved": False}

    _collect(provider, factory, singapore_holidays=(christmas, deepavali))

    backend = factory.created[0]
    assert json.loads(backend.files[REFERENCE_SINGAPORE_HOLIDAY_ENTRIES]) == [deepavali, christmas]


def test_reference_sources_are_read_from_disk_once_per_process():
    load_user_guide_references.cache_clear()
    load_singapore_holidays_reference.cache_clear()

    first_guide = load_user_guide_references()
    first_holidays = load_singapore_holidays_reference()

    assert load_user_guide_references() is first_guide
    assert load_singapore_holidays_reference() is first_holidays
