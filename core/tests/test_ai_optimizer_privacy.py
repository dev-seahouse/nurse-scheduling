...
def test_optimizer_submission_anonymizes_shift_type_covering_people() -> None:
    payload = parse_schedule(named_schedule())
    payload["preferences"].append(
        {"type": "shift type covering", "preceptors": [["Alice", "STAFF"]], "preceptees": ["Bob"], "shiftTypes": ["D"]}
    )

    outbound = parse_schedule(prepare_optimizer_schedule(schedule_yaml(payload), 1_000_000).submission_yaml)

    assert outbound["preferences"][-1]["preceptors"] == [["P2", "STAFF"]]
    assert outbound["preferences"][-1]["preceptees"] == ["P3"]
