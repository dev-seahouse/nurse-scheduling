...
def test_session_singapore_holidays_reach_the_sandbox_and_can_be_refreshed() -> None:
    read_entries = [
        ToolCallRequest((ToolCall("call_0", READ_TOOL, json.dumps({"path": "/reference/singaporeHolidays.json"})),))
    ]
    provider = ScriptedToolProvider(read_entries, [TextDelta("Read.")])
    factory = FakeSandboxFactory()
    client = AuthenticatedTestClient(
        create_test_app(settings=make_settings(), provider=provider, sandbox_factory=factory)
    )
    christmas = {"date": "2025-12-25", "name": "Christmas Day", "isObserved": False}
    new_year = {"date": "2026-01-01", "name": "New Year's Day", "isObserved": False}
    created = client.post("/sessions", json={"schedule_yaml": "description: test", "singapore_holidays": [christmas]})
    assert created.status_code == 201
    session_id = created.json()["id"]
    refreshed = client.put(
        f"/sessions/{session_id}/schedule",
        json={"schedule_yaml": "description: test", "singapore_holidays": [new_year, christmas]},
    )
    assert refreshed.status_code == 204

    response = client.post(f"/sessions/{session_id}/messages", json={"message": "Read the holidays."})

    assert response.status_code == 200
    assert json.loads(factory.created[0].files["/reference/singaporeHolidays.json"]) == [christmas, new_year]


def test_session_rejects_malformed_singapore_holidays() -> None:
    client = AuthenticatedTestClient(create_test_app(settings=make_settings(), provider=FakeProvider()))

    response = client.post(
        "/sessions",
        json={"schedule_yaml": "description: test", "singapore_holidays": [{"date": "25/12/2025", "name": "X"}]},
    )
