...
from nurse_scheduling.ai.schema import (
    ...
    SINGAPORE_HOLIDAYS_SOURCE,
    USER_GUIDE_DIRECTORY,
    load_schedule_reference,
    load_singapore_holidays_reference,
...
MODEL_TOPIC_COVERAGE = {
    ...
    models.HoursContractMetadata: ("preferences.shift count.hoursContract",),
    models.ShiftTypeCoveringPreference: ("preferences.shift type covering",),
...
CANONICAL_FIELD_REQUIREMENTS = {
    ...
    "shiftTypes.items": {
        ...
        "optional": ("durationMinutes", "startTime", "endTime", "restMinutes"),
    },
    ...
    "preferences.shift count": {
        ...
        "optional": ("description", "countShiftTypeCoefficients", "weight", "hoursContract"),
    },
    "preferences.shift count.hoursContract": {"required": ("unit", "policy")},
    ...
    "preferences.shift type covering": {
        "write": ("type",),
        "required": ("preceptors", "preceptees", "shiftTypes"),
        "optional": ("date", "description", "weight"),
    },
...
def test_singapore_holiday_reference_is_the_frontend_source():
    assert load_singapore_holidays_reference() == SINGAPORE_HOLIDAYS_SOURCE.read_text(encoding="utf-8")
    assert "buildSingaporeHolidayGroups" in load_singapore_holidays_reference()
