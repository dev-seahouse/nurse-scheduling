...
# A data.gov.sg snapshot that stands in for the entries the browser caches and sends.
SINGAPORE_HOLIDAYS_FIXTURE = Path(__file__).resolve().parent / "fixtures" / "singapore-holidays.json"
DEFAULT_CASE_JOBS = 4
...
async def run_case(
    ...
    singapore_holidays = json.loads(SINGAPORE_HOLIDAYS_FIXTURE.read_text(encoding="utf-8"))
    try:
...
def _reference_digests() -> dict[str, str]:
    ...
    The user guide pages, the Singapore holiday source, and its entry snapshot now steer
    the agent too, so an edit to one of them has to be visible when comparing two runs.
    """
    digests = {
    ...
    digests[SINGAPORE_HOLIDAYS_SOURCE.name] = hashlib.sha256(load_singapore_holidays_reference().encode()).hexdigest()
    digests[SINGAPORE_HOLIDAYS_FIXTURE.name] = hashlib.sha256(SINGAPORE_HOLIDAYS_FIXTURE.read_bytes()).hexdigest()
