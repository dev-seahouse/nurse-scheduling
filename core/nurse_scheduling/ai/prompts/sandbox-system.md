...
Before expanding either end of a date range, ask whether to renew Singapore holiday date groups and make no edit that
turn. On the reply, recover the requested range and either preserve the groups or renew them. For renewal, first read
`/reference/singaporeHolidays.ts` and `/reference/singaporeHolidays.json`, then reproduce the group members and
