...
class SingaporeHolidayEntry(BaseModel):
    """One browser-cached data.gov.sg public holiday, as `singaporeHolidays.ts` stores it."""
...
# data.gov.sg returns about 200 rows, so this bounds a session without truncating real data.
MAX_SINGAPORE_HOLIDAY_ENTRIES = 1000
...
class UpdateScheduleRequest(BaseModel):
    ...
    # None keeps the holidays the session already holds.
    singapore_holidays: list[SingaporeHolidayEntry] | None = Field(
        default=None, max_length=MAX_SINGAPORE_HOLIDAY_ENTRIES
    )
...
class CreateSessionRequest(BaseModel):
    ...
    schedule_yaml: str = Field(min_length=1)
    singapore_holidays: list[SingaporeHolidayEntry] = Field(
        default_factory=list, max_length=MAX_SINGAPORE_HOLIDAY_ENTRIES
    )
...
class ChatSession:
    ...
    # ponytail: not charged to the text capacity, bounded by MAX_SINGAPORE_HOLIDAY_ENTRIES instead.
    singapore_holidays: tuple[dict[str, object], ...] = ()
...
class SessionStore:
    ...
    def create(
        ...
        owner_token: str,
        schedule_yaml: str,
        singapore_holidays: Sequence[dict[str, object]] = (),
    ) -> ChatSession:
        ...
        with self._lock:
            ...
            session = ChatSession(
                ...
                singapore_holidays=tuple(singapore_holidays),
            )
    ...
    def singapore_holidays(self, session_id: str) -> tuple[dict[str, object], ...]:
        """Return the holiday entries the browser supplied, for sandbox hydration."""
        with self._lock:
            session = self._sessions.get(session_id)
            return session.singapore_holidays if session is not None else ()
    ...
    def update_schedule(
        ...
        owner_token: str | None,
        schedule_yaml: str,
        singapore_holidays: Sequence[dict[str, object]] | None = None,
    ) -> None:
        """Replace the schedule snapshot, which drops any proposal made against the old one."""
        with self._lock:
            session = self._get_owned(session_id, owner_token)
            session.expires_at = time.monotonic() + self._settings.session_ttl_seconds
            if singapore_holidays is not None:
                session.singapore_holidays = tuple(singapore_holidays)
    ...
    *,
    ...
    async def create_session(
        ...
        session = store.create(
            owner,
            request.schedule_yaml,
            [entry.model_dump() for entry in request.singapore_holidays],
        )
        logger.info(
            ...
            try:
                ...
                async with concurrency_limit:
                    agent_events = run_sandbox_agent(
                        ...
                        singapore_holidays=store.singapore_holidays(session_id),
                    )
    ...
    async def update_schedule(
        ...
        store.update_schedule(
            ...
            request.schedule_yaml,
            None
            if request.singapore_holidays is None
            else [entry.model_dump() for entry in request.singapore_holidays],
        )
