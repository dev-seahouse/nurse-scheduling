...
- Expanding means moving `startDate` earlier or `endDate` later. Before any expansion, ask whether to renew the
  Singapore holiday groups and wait for an explicit answer. Do not edit the schedule in the question turn.
...
- If Singapore holiday renewal is declined, preserve every existing date group and migrate only concrete member IDs as
  required by the range's new ID format. Do not add newly covered dates to WORKDAY, NON-WORKDAY, or PH.
- If Singapore holiday renewal is accepted, read `/reference/singaporeHolidays.ts` and
  `/reference/singaporeHolidays.json`. The JSON holds the data.gov.sg entries that the browser loaded, and the
...
- required `id`: string
- required `description`: string, which may be empty
- optional `history`: flat list of shift-type IDs, OFF, or LEAVE

Rules:
- Person IDs must be unique and cannot case-insensitively equal the reserved selector ALL.
- History may contain concrete shift-type IDs, OFF, or LEAVE, but not ALL or shift-type group IDs.
...
- required `id`: string
- required `description`: string, which may be empty
- optional `durationMinutes`: positive integer, the paid working minutes, a multiple of 30
- optional `startTime`: HH:00 or HH:30 clock time
- optional `endTime`: HH:00 or HH:30 clock time. An earlier end time means an overnight shift
- optional `restMinutes`: unpaid break minutes, a positive multiple of 30

Rules:
- Shift-type IDs must be unique and cannot case-insensitively equal the reserved selectors ALL, OFF, or LEAVE.
- The working-time fields are authoring data. The solver ignores them, so preserve them unless the user asks.
- Write `startTime` and `endTime` together. When they are set, `durationMinutes` is required and must equal the span
  minus `restMinutes`. `restMinutes` requires both clock times. Without clock times, `durationMinutes` may stand alone.
...
Rules:
- Group IDs must be unique across shift types and groups and cannot case-insensitively equal the reserved selectors ALL, OFF, or LEAVE.
