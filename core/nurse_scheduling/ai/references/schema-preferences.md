...
- preferences.shift type covering
...
- optional `weight`: integer or infinity, default -1
- optional `description`: string
- optional `hoursContract`: mapping that marks a contracted-hours count, see preferences.shift count.hoursContract

Rules:
- `countShiftTypes` must resolve to at least one shift type.
- With `hoursContract`, the target counts half-hours and `weight` must be `.inf`. The `exact` policy uses
  `expression: x = T` with one target. The `range` policy uses `expression: ['x >= T', 'x <= T']` with a
  [minimum, maximum] target. Every selected concrete shift type, and LEAVE when selected, needs its own coefficient.
  OFF, ALL, and group IDs are not allowed as coefficients.
...
Path: preferences.shift count.hoursContract

Marker for a shift count that enforces contracted hours.

Fields:
- required `unit`: half-hour
- required `policy`: exact or range

Rules:
- Preserve an existing marker. Add one only when the user asks for contracted hours.
...
Path: preferences.shift type covering

Require a preceptor on shift whenever a preceptee works it.

Fields:
- write `type`: shift type covering
- optional `date`: flat list of date or date-group IDs. Omit it to cover every date
- required `preceptors`: list of person or people-group IDs. A nested list is one group
- required `preceptees`: list of person or people-group IDs. A nested list is one group
- required `shiftTypes`: list of shift-type or shift-type-group IDs. A nested list is one group
- optional `weight`: integer or infinity, default 1
- optional `description`: string

Rules:
- The rule is hard. When any preceptee works a selected shift on a date, a preceptor must work it too.
- Each top-level preceptor entry must cover on its own. Write `[[A, B]]` for at least one of A or B. `[A, B]` requires both.
- `shiftTypes` cannot contain OFF or LEAVE.
