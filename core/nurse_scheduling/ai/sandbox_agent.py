...
REFERENCE_SCHEMAS["singapore-holidays"] = f"/reference/{SINGAPORE_HOLIDAYS_SOURCE.name}"
# Browser-cached data.gov.sg entries, the data that the builder above consumes.
REFERENCE_SINGAPORE_HOLIDAY_ENTRIES = "/reference/singaporeHolidays.json"
...
async def _measured_sandbox_turn(
    ...
    singapore_holidays: Sequence[Mapping[str, object]],
...
class _LazySandboxTurn:
    ...
    def __init__(
        ...
        self._singapore_holidays = tuple(singapore_holidays)
...
async def hydrate_sandbox(
    ...
    for group, path in REFERENCE_SCHEMAS.items():
        reference = (
            load_singapore_holidays_reference() if group == "singapore-holidays" else load_schedule_reference(group)
        )
    ...
    files[REFERENCE_SINGAPORE_HOLIDAY_ENTRIES] = json.dumps(
        sorted(singapore_holidays, key=lambda entry: str(entry["date"])),
        ensure_ascii=False,
        indent=2,
    )
