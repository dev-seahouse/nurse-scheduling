...
SINGAPORE_HOLIDAYS_SOURCE = Path(__file__).resolve().parents[3] / "web-frontend/src/utils/singaporeHolidays.ts"
...
@lru_cache(maxsize=1)
def load_singapore_holidays_reference() -> str:
    """Load the frontend's authoritative Singapore holiday group builder.

    The holiday entries themselves come from the browser, see `hydrate_sandbox`.
    """
    reference = SINGAPORE_HOLIDAYS_SOURCE.read_text(encoding="utf-8")
    if len(reference) > MAX_SCHEMA_REFERENCE_CHARS:
        raise ValueError(f"Singapore holiday reference exceeds {MAX_SCHEMA_REFERENCE_CHARS} characters")
    return reference
