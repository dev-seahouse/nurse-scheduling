...
An unconfirmed request cleanup is logged as a warning.
...
cleanup is logged as confirmation.
...
| `POST /sessions` | Store a YAML snapshot and the optional `singapore_holidays` list, then create a browser-owned session. |
...
attachment field names are rejected.

The browser sends its cached data.gov.sg entries as `singapore_holidays`. Each
entry has `date`, `name`, and `isObserved`. `PUT /sessions/{id}/schedule`
accepts the same field to refresh the list. Every turn writes the list to
`/reference/singaporeHolidays.json` beside `/reference/singaporeHolidays.ts`,
so that the assistant can rebuild the `WORKDAY`, `NON-WORKDAY`, and `PH` groups. Sessions are process-local. Use one AI
