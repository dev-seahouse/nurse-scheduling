...
Submitted YAML is bounded by how far
...
In Compose, NGINX replaces `X-Forwarded-For` with Cloudflare's single
`CF-Connecting-IP` value, and Uvicorn trusts that forwarded header. The original
caller-supplied forwarding chain is intentionally discarded.
