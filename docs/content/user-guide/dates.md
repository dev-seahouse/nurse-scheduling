...
`NON-WORKDAY` separate staffing days. `Before 4` and `After 4` let later rules use
...
## Import Singapore calendar groups

Holiday import can create or replace the editable groups `WORKDAY`,
`NON-WORKDAY`, and `PH`. `PH` holds the gazetted public holidays, including
observed days. `NON-WORKDAY` holds the public holidays and the weekends, and
`WORKDAY` holds the other days. Use the import only when these groups match the
workplace calendar. The app loads the holidays from data.gov.sg, and the Dates
page shows the supported range.
